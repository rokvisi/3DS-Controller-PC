...
int UDP_Server::recv(char* const data, const size_t size) {
    ...
    #else
        return static_cast<int>(recvfrom(connection, data, size, 0, addr_info->ai_addr, &addr_info->ai_addrlen));
    #endif
}
int UDP_Server::recv(std::string& message) {
    ...
    #else
        return static_cast<int>(recvfrom(connection, &message[0], message.size(), 0, addr_info->ai_addr, &addr_info->ai_addrlen));
    #endif
}
...
int UDP_Client::send(const std::string& message) {
    return static_cast<int>(::sendto(connection, message.data(), message.size(), 0, addr_info->ai_addr, addr_info->ai_addrlen));
}
int UDP_Client::send(const char* const data, const size_t size) {
    return static_cast<int>(::sendto(connection, data, size, 0, addr_info->ai_addr, addr_info->ai_addrlen));
}
